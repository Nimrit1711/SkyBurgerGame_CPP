...
#include "Sock.h"
#include "GameMode.h" // Include GameMode header

using namespace sf;
using namespace std;

// Compile code: g++ HazardsSpawnTest.cpp Player.cpp Burger.cpp FoodItem.cpp Tomato.cpp Patty.cpp Bomb.cpp Hazards.cpp Cheese.cpp Onion.cpp Sock.cpp BananaPeel.cpp PoisonBottle.cpp -lsfml-graphics -lsfml-window -lsfml-system

int main() {
    // Seeds random number generator
    srand(static_cast<unsigned int>(time(0)));

    RenderWindow window(VideoMode(900, 800), "Spawning Object Test");

    Burger burger;
    Player player(&burger);

    // Create an instance of GameMode and set the desired mode
    GameMode gameMode("default");
    gameMode.applyMode(); // Apply the settings based on the mode
    ...
    float spawnInterval = gameMode.getHazardSpawnRate(); // Get spawn interval from game mode
    ...
    while (window.isOpen() && gameRunning) {
        ...
        if (spawnTimer >= spawnInterval) {
            ...
            // Set the ingredients starting position with random X
            newItem->setPosition(Vector2f(randomX, 0.0f));

            // Set fall speed multiplier from game mode
            newItem->setFallSpeed(newItem->getFallSpeed() * gameMode.getFallSpeedMultiplier());
        ...
        for (auto it = fallingItems.begin(); it != fallingItems.end(); ) { 
            (*it)->update(deltaTime);
            if (auto foodItem = dynamic_cast<FoodItem*>(*it)) { // if it's a food item
                ...
                if (hazard->getIsCaught()) {
                    if (auto poisonBottle = dynamic_cast<PoisonBottle*>(hazard)) { // checks if it's the poison bottle
                        poisonBottle->applyPoisonEffect(player);
                    } else {
                        player.loseLife(); // if it's the other hazards, lose a life. 
                    }
